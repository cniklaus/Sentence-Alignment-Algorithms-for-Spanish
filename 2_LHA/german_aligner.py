...
#parser.add_argument('-src_sents', help='The source splitted sents file .eoa', required=False)

#parser.add_argument('-tgt_sents', help='The target splitted sents file .eoa', required=False)
...
if __name__ == '__main__':
    ...
    args_dict['w2v'] = w2v
    args_dict["w2v_model_path"] = "models/cc.es.300.vec"
    ...
    count = 930
    while count < 3390:
        src = f"{corpus}/doc_level_files/docs_{level_complex}/{count}.txt"
        tgt = f"{corpus}/doc_level_files/docs_{level_simple}/{count}.txt"
        src_sents = f" {corpus}/{level_complex}_final/{count}.txt"
        tgt_sents = f"{corpus}/{level_simple}_final/{count}.txt"


        args_dict['src'] = f"{corpus}/doc_level_files/docs_{level_complex}/{count}.txt"
        args_dict['tgt'] = f"{corpus}/doc_level_files/docs_{level_simple}/{count}.txt"
        args_dict['src_sents'] = f"{corpus}/{level_complex}_final/{count}.txt"
        args_dict['tgt_sents'] = f"{corpus}/{level_simple}_final/{count}.txt"
        args_dict["w2v_model_path"] = "models/cc.es.300.vec"
        args_dict["vec_size"] = "300"
        args_dict['emb'] = 'avg'
        args_dict['level'] = 'local'
        args_dict['rolling_thresholds'] = 0.5
        args_dict['gold_src'] = ''
        args_dict['gold_tgt'] = ''


        try:
            total_documents_src = wc(args_dict['src']) #wc(src) #wc(args_dict['src'])
            total_documents_tgt = wc(args_dict['src']) #wc(tgt) #wc(args_dict['tgt'])
        ...
        for key in ["src", "tgt", "src_sents", "tgt_sents"]:
            del args_dict[key]
